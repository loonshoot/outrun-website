...
const GET_CONFLUENCE_TOKENS = gql`
  query GetConfluenceTokens($workspaceSlug: String, $service: String) {
...
export default function ConfluenceSourcePage() {
  ...
  const [service] = useState("confluence");
  const [currentStep, setCurrentStep] = useState(1);
  const [selectedTokenId, setSelectedTokenId] = useState(null);
  const [sourceName, setSourceName] = useState('Confluence');
  ...
  // Required scopes array definition for Confluence
    ...
    if (code && !isRegistering && registrationAttemptCount < MAX_REGISTRATION_ATTEMPTS && workspace) {
      ...
      .then(result => {
        ...
        if (result.data && result.data.registerExternalCredentials && result.data.registerExternalCredentials.remainingTokens) {
          ...
          toast.success('Confluence account added successfully');
        }
        setIsPolling(true);
        setPollingCount(0);
        setCode(null);
      ...
      toast.error('Failed to connect to Confluence after multiple attempts. Please try again.');
      setCode(null);
    }
  ...
  const handleRefreshToken = async (tokenId) => {
    try {
      ...
      toast.info('Refreshing Confluence account...');
      ...
      if (result.data?.refreshExternalCredentials?.remainingTokens) {
        ...
        toast.success('Confluence account refreshed successfully');
      }
    } catch (error) {
  ...
  // Function to get Confluence OAuth URL
  const getConfluenceAuthUrl = () => {
    // Fetch the clientId from server
    return fetch(`/api/confluence-client-id`)
      .then(response => response.json())
      .then(data => {
        const confluenceClientId = data.clientId;
        const currentUrl = `${window.location.origin}/${workspaceSlug}/sources/add/confluence`;
        const redirectUri = `${window.location.origin}/api/callback/confluence`;
        
        const params = new URLSearchParams({
          client_id: confluenceClientId,
      ...
      .catch(error => {
        console.error("Error fetching Confluence client ID:", error);
        toast.error("Failed to get Confluence authorization URL");
      });
  };
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      toast.success(
        'Records are now being synchronized from Confluence. Initial sync is running in the background.',
        { 
          duration: 8000,
          icon: '🔄'
        }
  ...
  return (
    <AccountLayout routerType="pages">
      <Meta title={`Outrun - ${workspace?.name || 'Dashboard'} | ${t("sources.add.title.confluence") || "Add Confluence Source"}`} />
      <Content.Title
        title={t("sources.add.title") || "Add Source"}
        subtitle={t("sources.add.subtitle.confluence") || "Connect to Confluence"}
      />
      <Content.Divider />
      <Content.Container>
        ...
        {currentStep === 1 && (
          <Card>
            <Card.Body
              title={t("sources.add.step1.title.confluence") || "1. Select Confluence Account"}
            ...
            >
              {tokens && tokens.length > 0 ? (
                        ...
                        <td className="py-3">
                          <div className="flex flex-row items-center justify-end space-x-3">
                            {hoveredTokenId === token._id && (
                              ...
                              >
                                {t('sources.add.confluence.refreshCredential') || 'Refresh Credential'}
                              </button>
                            )}
                ...
                <p className="mt-4 font-bold text-light">
                  {t('sources.add.confluence.addFirstAccount') || "Add your first Confluence account below"}
                </p>
              )}
              <div className="mt-4">
                <Button
                  background="Yellow"
                  border="Light"
                  onClick={getConfluenceAuthUrl}
                >
                  {t('sources.add.confluence.addAccount') || 'Add Account'}
                </Button>
              </div>
            </Card.Body>
            <Card.Footer>
              ...
              <table className="table-fixed">
                <thead className="text-light border-b">
                  <tr>
                    ...
                    <th className="py-3 text-left">{t("sources.confluence.cloudId") || "Cloud ID"}</th>
                  </tr>
                </thead>
                <tbody className="text-sm">
        ...
        {currentStep === 3 && (
          <Card>
            <Card.Body
              ...
              subtitle={t("sources.add.step3.subtitle") || "Configure your Confluence source"}
            >
              <div className="space-y-6">
                ...
                <div className="space-y-2">
                  <p className="text-sm text-amber-500">
                    {t("sources.add.confluence.backfillInfo") || "All historical data will be imported from Confluence."}
                  </p>
                </div>
              </div>
            </Card.Body>
